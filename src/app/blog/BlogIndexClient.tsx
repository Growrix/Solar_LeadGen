...
function ArticleCard({ article, onNavigateToPost }: ArticleCardProps) {
  return (
  <article
    ...
    onKeyDown={(e) => e.key === 'Enter' && onNavigateToPost(article)}
  >
            ...
            <div className="max-w-xl mx-auto mt-10">
              <div className="theme-card flex items-center gap-3 px-4 py-3">
                ...
                <input
                  ...
                  className="w-full bg-transparent outline-none text-body-small text-foreground placeholder:text-muted-foreground"
                />
              </div>
            </div>
          </div>
        ...
        <section className="py-16 sm:py-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-10 items-start">
                ...
                {filteredArticles.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {filteredArticles.slice(0, visibleCount).map((article, index) => (
                      <div key={`${article.slug ?? index}`} className="animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
              ...
              <BlogSidebarClient categories={categories} recentPosts={recentPosts} />
            </div>
          </div>
        </section>
      </main>
