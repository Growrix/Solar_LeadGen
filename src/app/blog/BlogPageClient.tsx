...
export default function BlogPageClient({ initialArticles }: { initialArticles: Post[] }) {
  const router = useRouter();
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('All');
  const [visibleCount, setVisibleCount] = useState(ARTICLES_PER_PAGE);

  const allArticles = useMemo(() => initialArticles, [initialArticles]);
  ...
  const handleNavigateToPost = (post: Post) => {
    const slug = (post.slug && typeof post.slug === 'string' ? post.slug : '') || slugify(post.title);
    if (!slug) return;
      ...
      <main className="flex-grow pb-24 md:pb-0">
        ...
        <section className="py-16 sm:py-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="theme-card p-6 mb-10">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-body-small text-muted-foreground mb-2">Search</label>
                  <input
