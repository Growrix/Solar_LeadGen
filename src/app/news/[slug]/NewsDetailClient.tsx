...
export default function NewsDetailClient({
  ...
  slug: string;
  initialItem: PublicNewsItemDetail | null;
}) {
  const router = useRouter();
  const [item, setItem] = React.useState<PublicNewsItemDetail | null>(initialItem);
  const [isLoading, setIsLoading] = React.useState(!initialItem);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const found = await fetchPublicNewsBySlug(slug);
        if (!alive) return;
        setItem(found);
      } catch {
        if (!alive) return;
  ...
  return (
    <div className="min-h-screen bg-background font-sans animate-in fade-in duration-700">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 lg:py-16">
        ...
        {/* Feature Visual */}
        <div className="aspect-[21/9] md:aspect-[16/7] rounded-3xl shadow-neu-outset overflow-hidden mb-12 relative bg-surface">
          {item.ogImageUrl ? (
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mt-16 flex flex-wrap gap-2.5">
            {item.tags?.length ? (
              ...
              <span className="text-body-small text-muted-foreground">No tags</span>
            )}
          </div>
