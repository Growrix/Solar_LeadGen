...
import { WrittenQuoteNegotiationPanel, WQEvent } from './written-quote/WrittenQuoteNegotiationPanel';
...
interface QuoteDraft {
  mode: 'quote' | 'bid' | 'config' | 'written-quote';
...
const QuoteBuilderModal: React.FC<QuoteBuilderModalProps> = ({
  ...
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    ...
    writtenQuoteNegotiation: true, // Expanded by default in written-quote mode
    leadDetails: false
  });
    ...
    try {
      if (mode === 'bid') {
        ...
        const writtenQuotePayload = {
          leadId: String(lead.id),
          initialPrice: finalTotal,
          notes: 'Initial written quote proposal',
          
          // Comprehensive Quote Builder data (same structure as Bid)
          systemData: {
            capacityKw: quoteDraft.system.systemSize,
            systemType: quoteDraft.system.systemType,
            solarPanelsArray: [{
          ...
          installerContact: {
            companyName: 'Installer Company', // TODO: Get from session
        ...
        // Clear draft on success
        const draftKey = `written-quote:draft:${lead.id}:installer-id`;
        localStorage.removeItem(draftKey);

        alert(`Written quote submitted successfully! Quote ID: ${data.writtenQuoteId}`);
        onClose();
      } else {
        // For regular quotes, use the existing onSubmitQuote handler
        const quoteData = {
          ...quoteDraft,
          leadId: lead.id
        };
            ...
            <div className="flex items-center gap-2 w-full md:w-auto flex-wrap">
              ...
              >
                ...
                {isSubmitting 
                  ? 'Submitting...' 
                  : mode === 'bid' ? 'Submit Bid'
                  : mode === 'written-quote' ? 'Submit Quote'
                  : 'Send Quote'}
              </Button>
              <Button
                onClick={onClose}
        ...
        <div className="flex-grow overflow-hidden flex gap-4 p-4 md:p-6">
          ...
          <div className="w-[30%] overflow-y-auto pl-2">
            <div className="sticky top-0 space-y-6">
              ...
              {/* Written Quote Negotiation Panel - Collapsible (only in written-quote mode) */}
              {mode === 'written-quote' && (
                <CollapsibleSection
                ...
                >
                  <WrittenQuoteNegotiationPanel
                    ...
                    onAction={async (action, data) => {
                      ...
                      // TODO: Wire to API in Sprint 2E (T-WQ-214)
