...
import { Plus, Edit2, Trash2, AlertCircle, RefreshCw, Tag as TagIcon, CheckCircle, Search, ArrowLeft } from 'lucide-react';
        ...
        <div className="bg-surface border-b border-border px-6 py-8">
          <div className="max-w-4xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-heading-2 text-foreground">Tags</h1>
                <p className="text-body-small text-foreground-muted mt-1">Manage tags to group related content.</p>
              </div>
            </div>
            <button
      ...
      <div className={`${isTabbed ? 'max-w-7xl' : 'max-w-4xl'} mx-auto px-6 py-8`}>
        ...
        {viewState === 'error' && (
          <div className="bg-surface rounded-card border border-error/20 p-8 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-heading-4 text-foreground mb-2">Could not load tags</h3>
        ...
        {(viewState === 'empty' || (viewState === 'success' && filteredTags.length === 0 && !searchQuery)) && (
          <div className="bg-surface rounded-card border border-border p-12 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-heading-4 text-foreground mb-1">No tags found</h3>
            <p className="text-body-small text-foreground-muted mb-6">Create tags to help users find your content.</p>
            <button 
              onClick={handleAddNew}
        ...
        {viewState === 'success' && filteredTags.length === 0 && searchQuery && (
          <div className="bg-surface rounded-card border border-dashed border-border p-12 flex flex-col items-center justify-center text-center">
            <p className="text-body-small text-foreground-muted">No tags found matching &quot;{searchQuery}&quot;</p>
            <button 
              ...
              <table className="min-w-full divide-y divide-border">
                ...
                <tbody className="bg-surface divide-y divide-border">
                  {filteredTags.map((tag) => (
                    <tr key={tag.id} className="hover:bg-muted transition-colors group">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <TagIcon className="icon-xs text-muted-foreground" />
                          <span className="text-body-small text-foreground">{tag.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-caption font-mono bg-muted text-foreground-muted px-2 py-1 rounded border border-border">{tag.slug}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
