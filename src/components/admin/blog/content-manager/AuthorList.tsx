      ...
      <div className={`${isTabbed ? 'max-w-7xl' : 'max-w-6xl'} mx-auto px-6 py-8 pb-32`}>
        ...
        {viewState === 'error' && (
          <div className="bg-white rounded-lg border border-red-100 p-8 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-medium text-slate-900 mb-2">Could not load authors</h3>
            <button
              onClick={fetchData}
              className="text-sm font-medium text-slate-600 hover:text-slate-900 underline flex items-center"
            >
        ...
        {viewState === 'success' && filteredAuthors.length === 0 && searchQuery && (
          <div className="bg-white border border-dashed border-slate-300 rounded-xl p-12 flex flex-col items-center justify-center text-center">
            <p className="text-slate-500">No authors found matching &quot;{searchQuery}&quot;</p>
            <button onClick={() => setSearchQuery('')} className="mt-2 text-solar-600 font-medium text-sm hover:underline">
              Clear search
            </button>
          </div>
        )}
                  ...
                  {filteredAuthors.map((author) => (
                    <tr key={author.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full mr-1.5 ${author.status === 'active' ? 'bg-green-500' : 'bg-slate-400'}`}
                          />
