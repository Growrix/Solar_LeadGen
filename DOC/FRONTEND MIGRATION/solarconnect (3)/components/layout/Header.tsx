...
export const Header: React.FC<HeaderProps> = ({ onNavigate, activePage }) => {
  ...
  const handleNavClick = (href: string) => {
    ...
    if (href.startsWith('#')) {
      ...
      if (activePage !== 'home') {
        ...
        // Handle explicit page navigations
        onNavigate(href);
  ...
  const isComponentsActive = ['components', 'layout-structure'].includes(activePage);
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`
                    text-sm uppercase tracking-wide font-medium transition-colors border-none bg-transparent cursor-pointer focus:outline-none 
                    ${isActive ? 'text-brand-500' : 'text-slate-300 hover:text-brand-400'}
                  `}
                >
                  {item.label}
                </button>
            ...
            <button
                ...
                className={`flex items-center gap-1.5 text-sm uppercase tracking-wide font-medium transition-colors border-none bg-transparent cursor-pointer focus:outline-none ${
                    isComponentsActive ? 'text-brand-500' : 'text-slate-300 hover:text-brand-400'
                }`}
            >
        ...
        <div className="md:hidden bg-slate-900 border-b border-white/10">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {NAV_ITEMS.map((item) => {
               ...
               return (
                  <button
                    ...
                    onClick={() => handleNavClick(item.href)}
                    className={`
                      block w-full text-left px-3 py-2 rounded-md text-base font-medium focus:outline-none focus:bg-slate-800
                      ${isActive 
                        ? 'text-brand-500 bg-slate-800' 
            ...
            <button
                ...
                className={`block w-full text-left px-3 py-2 rounded-md text-base font-medium hover:text-white hover:bg-slate-800 focus:outline-none focus:bg-slate-800 ${
                  isComponentsActive ? 'text-brand-500' : 'text-slate-300'
                }`}
            >
