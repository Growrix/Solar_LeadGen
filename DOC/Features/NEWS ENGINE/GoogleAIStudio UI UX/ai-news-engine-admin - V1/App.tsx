...
import { NewsStatus, NewsItem, KPI, AppView, RSSSource } from './types';
...
}> = ({ isLoading, news, searchTerm, setSearchTerm, handleNewsAction }) => {
  const kpis: KPI[] = [
    ...
    { label: 'Automations', value: 'Active', trend: 0, icon: <Zap size={20} />, description: 'System status' },
  ...
  return (
    <div className={`flex h-screen overflow-hidden text-slate-900 ${isPublicView ? 'bg-white' : 'bg-slate-50'}`}>
      ...
      <main className={`flex-1 flex flex-col h-full overflow-y-auto ${isPublicView ? 'bg-white' : ''}`}>
        ...
        {!isPublicView && (
          <header className="sticky top-0 z-10 bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between shadow-sm">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => handleAction('Test & Preview')} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 shadow-sm">
                <RefreshCcw size={16} /> Test & Preview
              </button>
              <button 
                onClick={() => handleAction('Pause Automation')}
        ...
        {/* View Router */}
        {currentView === 'dashboard' && <DashboardView isLoading={isLoading} news={news} searchTerm={searchTerm} setSearchTerm={setSearchTerm} handleNewsAction={handleNewsAction} />}
        ...
        {currentView === 'control' && <ControlPage />}
      ...
      <TestPreviewModal isOpen={isTestPreviewModalOpen} onClose={() => setIsTestPreviewModalOpen(false)} />
