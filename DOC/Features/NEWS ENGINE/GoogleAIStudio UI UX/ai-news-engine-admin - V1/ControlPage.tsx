...
const ControlPage: React.FC = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setFeatures([
        { id: '1', name: 'RSS Ingestion', description: 'Monitor and fetch raw data from configured RSS feeds.', isActive: true, status: 'healthy', lastActivity: '2m ago' },
        { id: '2', name: 'Web Research Engine', description: 'AI agents exploring secondary sources and fact-checking.', isActive: true, status: 'healthy', lastActivity: '5m ago' },
        { id: '3', name: 'Drafting Engine', description: 'Generative AI creating structured news articles.', isActive: true, status: 'warning', lastActivity: '12m ago' },
        ...
        { id: '5', name: 'Social Distribution', description: 'Automated posting to LinkedIn, X, and internal channels.', isActive: true, status: 'healthy', lastActivity: '1h ago' },
        { id: '6', name: 'Deduplication Service', description: 'Semantic analysis to prevent covering the same story twice.', isActive: true, status: 'degraded', lastActivity: 'Just now' },
      ]);
      setIsLoading(false);
    }, 800);
  ...
  const handleGlobalAction = (action: string) => {
    console.log(`Global System Action: ${action}`);
    // State updates would happen here after confirmation
  };
      ...
      <section className="bg-slate-900 rounded-2xl p-8 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-8">
          <div className="space-y-2 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/20 text-emerald-400 rounded-full text-xs font-bold uppercase tracking-widest border border-emerald-500/30 mb-2">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <button 
              onClick={() => handleGlobalAction('Resume All')}
              className="flex items-center gap-2 px-6 py-3 bg-white text-slate-900 rounded-xl font-bold text-sm hover:bg-slate-50 transition-all active:scale-95 shadow-lg"
            >
              <PlayCircle size={20} fill="currentColor" />
              Resume All
            </button>
            <button 
              onClick={() => handleGlobalAction('Pause All')}
              className="flex items-center gap-2 px-6 py-3 bg-slate-800 text-white rounded-xl font-bold text-sm hover:bg-slate-700 border border-slate-700 transition-all active:scale-95"
            >
              <PauseCircle size={20} />
              Pause Pipeline
            </button>
            <button 
              onClick={() => handleGlobalAction('Emergency Stop')}
              className="flex items-center gap-2 px-6 py-3 bg-rose-600 text-white rounded-xl font-bold text-sm hover:bg-rose-700 transition-all active:scale-95 shadow-lg shadow-rose-900/40"
            >
              <ShieldAlert size={20} />
              Emergency Stop
            </button>
