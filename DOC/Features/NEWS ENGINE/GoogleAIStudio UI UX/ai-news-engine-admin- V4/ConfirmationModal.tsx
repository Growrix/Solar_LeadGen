...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({ 
  ...
  const handleConfirm = () => {
    ...
    // Add a tiny delay for deterministic feedback
    setTimeout(() => {
      onConfirm();
    }, 400);
  };
          ...
          {requireConfirmText && (
            <div className="space-y-3 pt-2 animate-in fade-in duration-500">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">
                Type <span className="text-slate-900 font-black">"{requireConfirmText}"</span> to confirm
              </label>
              <input 
                autoFocus
                type="text"
                ...
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-center text-sm font-bold focus:outline-none focus:ring-4 focus:ring-indigo-500/5 transition-all"
                placeholder="..."
              />
            </div>
          )}
