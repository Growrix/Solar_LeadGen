      ...
      <div className="relative bg-white w-full max-w-md rounded-2xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-300">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-xs font-bold text-slate-500 flex items-center gap-1.5 uppercase tracking-wider">
              <AlertCircle size={14} /> Rejection Reasons (Optional)
            </label>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 flex items-center gap-1.5 uppercase tracking-wider">
              <MessageSquare size={14} /> Feedback for AI/Admin
            </label>
            <textarea 
              value={reason}
              onChange={(e) => {
              ...
              placeholder="Provide detailed feedback on why this draft is being rejected..."
              className={`w-full h-24 px-4 py-3 bg-slate-50 border ${error ? 'border-rose-300' : 'border-slate-200'} rounded-xl text-sm focus:outline-none focus:ring-2 ${error ? 'focus:ring-rose-500/10' : 'focus:ring-indigo-500/10'} focus:border-indigo-500 transition-all resize-none`}
            />
            {error && (
        ...
        <footer className="px-6 py-4 bg-slate-50 rounded-b-2xl border-t border-slate-100 flex items-center justify-end gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-6 py-2 bg-rose-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-rose-500/20 hover:bg-rose-700 transition-all active:scale-[0.98] disabled:opacity-50"
          >
            {isSubmitting ? (
